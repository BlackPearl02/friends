...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    locale: raw,
    path: "/no-join-code",
    title: messages.meta.noJoinCodeTitle,
    description: messages.meta.noJoinCodeDescription,
    ogImageAlt: messages.meta.ogImageAlt,
  });
}

export default async function NoJoinCodePage({ params }: Props) {
  const { locale: raw } = await params;
  if (!isLocale(raw)) notFound();
  const m = getMessages(raw);
  const P = m.noJoinCode;
  ...
  return (
    <SeoContentPage
      ...
      title={P.title}
      lead={P.lead}
      breadcrumbLabel={m.footer.noJoinCode}
      ...
      related={getRelatedSeoLinks(m, "/no-join-code")}
    />
  );
}
