...
1. Fork the repository (or use a branch on a collaborator clone). Prefer PRs into `dev` for Preview deploys; promote to `main` for Production.
...
Copy `.env.example` to `.env.development` for local Docker staging. Production values live in gitignored `.env.production` and Vercel **Production** scope — see [docs/deployment.md](docs/deployment.md#branches--environments). Never paste real secrets into issues, PRs, or screenshots.
